...
import sg.edu.nus.iss.sereserch.ethan.listener.GestureImageViewListener;
import sg.edu.nus.iss.sereserch.ethan.utils.MathUtils;
import sg.edu.nus.iss.sereserch.ethan.utils.VectorF;
import sg.edu.nus.iss.sereserch.ethan.view.animation.FlingAnimation;
import sg.edu.nus.iss.sereserch.ethan.listener.FlingAnimationListener;
import sg.edu.nus.iss.sereserch.ethan.listener.FlingListener;
import sg.edu.nus.iss.sereserch.ethan.view.animation.MoveAnimation;
import sg.edu.nus.iss.sereserch.ethan.listener.MoveAnimationListener;
import sg.edu.nus.iss.sereserch.ethan.view.animation.ZoomAnimation;
import sg.edu.nus.iss.sereserch.ethan.listener.ZoomAnimationListener;
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    if (event.getPointerCount() > 1) {
                        ...
                        if (initialDistance > 0) {
                            ...
                            float distance = (float) pinchVector.getLength();

                            if (initialDistance != distance) {

                                float newScale = (float) ((distance / initialDistance) * lastScale);

                                if (newScale <= maxScale) {
                                    scaleVector.setLength(scaleVector.getLength() * newScale);

                                    scaleVector.calculateEndPoint();

                                    scaleVector.setLength(scaleVector.getLength() / newScale);

                                    float newX = scaleVector.getEnd().x;
                                    float newY = scaleVector.getEnd().y;
                            ...
                            scaleVector.setLength(scaleVector.getLength() / lastScale);
                        }
                    } else {
                        if (!touched) {
                            touched = true;
