...
public class RoundKnobButton extends RelativeLayout implements OnGestureListener {
	...
	public RoundKnobButton(Context context,AttributeSet attr)
	{
		this(context, attr, R.drawable.stator, R.drawable.rotoron, R.drawable.rotoroff);

	}
	public RoundKnobButton(Context context,AttributeSet attr, int back, int rotoron, int rotoroff)  {
		super(context,attr);
		//int back, rotoron, rotoroff, w,   h;
		// we won't wait for our size to be calculated, we'll just store out fixed size
		Singleton m_Inst = Singleton.getInstance();

		//back=R.drawable.stator;rotoron= R.drawable.rotoron;rotoroff= R.drawable.rotoroff;
				int w=m_Inst.Scale(96); int h= m_Inst.Scale(96);
		m_nWidth = w; 
		m_nHeight = h;
		// create stator
		...
		int x=srcon.getWidth();
		int y=srcon.getHeight();
