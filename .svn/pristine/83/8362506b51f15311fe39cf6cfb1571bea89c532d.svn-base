package sg.edu.nus.iss.sereserch.ethan.view.animation;
...
import sg.edu.nus.iss.sereserch.ethan.view.ZoomImageView;
    ...
    public boolean update(ZoomImageView view, long time) {
        if (firstFrame) {
            ...
            if (scaleDiff > 0) {
                ...
                vector.setLength(length * zoom);

                // Now deduce the new endpoint
                vector.calculateEndPoint();

                xDiff = vector.getEnd().x - startX;
                yDiff = vector.getEnd().y - startY;
            } else {
                // Zoom out to center
